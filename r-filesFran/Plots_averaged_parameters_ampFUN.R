...
# Load data
load("~/Dropbox/ampOddClick/r-filesFran/Figures/Summary_parameters/ampFUN/EEG_Mega_variable_param_v2")
...
Plot_1 <- ggplot()+
  ...
  ggtitle(paste("Recovery time constant",sep=""))
...
Plot_2 <- ggplot()+
  geom_col(data = dfR, aes(x = reorder(Intensity,order), y = Mean))+
  geom_errorbar(data = dfR, aes(x = Intensity, ymin=Mean-se, ymax=Mean+se, color = "green"), width=.1) +
  ggtitle(paste("Cumulative release probability",sep=""))
...
Final_plot <- Plot_1 + Plot_2 + Plot_3 + Plot_4 + Plot_5
