load_ampOddClick_Human <- function(Wave, Subject, Type_signal, Hemisphere,Parcel){
# Originally like this: load_ampOddClick_Human <- function(Wave='P2', Subject='2005',  Type_signal='EEG', Hemisphere='L',Parcel='A1'){  
  ...
  ST$logISI <- log2(ST$ISI)
  # ST$ampFct <- as.factor(ST$ampInd)
  ST$ampFct <- as.factor(ST$clickIntensity)
  ...
  ST$toneType           <- c(1,0)[1 + (ST$clickIntensity == ST$clickIntensityLag1)]
