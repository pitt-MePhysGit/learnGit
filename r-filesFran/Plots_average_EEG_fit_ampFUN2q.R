...
for (w in 1:length(Wave_list)) {
  ...
  All_se_pred <- matrix(list(), length(Subject_list), 15)
      ...
      for (subid in 1:length(Subject_list)) {
        ...
        if (file.exists(paste("/Volumes/EEGCyno/RES/ampOddClick/STPSP/STPSPFit",Subject, "22223344_5566_zm9999_all_DV_ampFUN_p2q",Subject,Parcel,Hemisphere,Wave,"gd.rda",sep="_"))){
          ...
          tst <- callSTPSP(tn, DVstr='DV', gd=T, show=TRUE, showCh=1,FUN=ampFUN2q,maxISI=10, minISI=0.2, maxp2p=5000, DVsdscale=5, 
                           maxEOG=Inf, trialType=NA, fitType=NA, manStartPar = NULL, robust = FALSE, startFromScratch = F,
      ...
      save(list=c("ISI_means_real", "ISI_se_real", "INT_means_real","INT_se_real","All_means_real","All_se_real","ISI_means_pred","ISI_se_pred","INT_se_pred","All_means_pred","All_se_pred"), file = paste("~/Dropbox/ampOddClick/r-filesFran/Summary_table_fits/ampFUN2q/",Type_signal,"_",Parcel,"_",Hemisphere,"_",Wave,sep=""))
...
ggsave(
  ...
  plot = Final_plot_sub,
  device = "png",
  path = "~/Dropbox/ampOddClick/r-filesFran/Figures/Model_fit/EEG/ampFUN2q/",
  scale = 2,
  width = 15,
  height = 5,
  units = "cm",
