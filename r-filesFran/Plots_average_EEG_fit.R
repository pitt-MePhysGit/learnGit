...
Generate_source_plots <- "YES" # Average not programmed for this yet
...
for (w in 1:length(Wave_list)) {
  ...
  All_se_pred <- matrix(list(), length(Subject_list), 15)
      ...
      ISI_means_real <- matrix(unlist(ISI_means_real), ncol = 5, byrow = TRUE)
      ISI_means_real <- colMeans(ISI_means_real)
      ISI_se_real <- matrix(unlist(ISI_se_real), ncol = 5, byrow = TRUE)
      ISI_se_real <- colMeans(ISI_se_real)
      INT_means_real <- matrix(unlist(INT_means_real), ncol = 3, byrow = TRUE)
      INT_means_real <- colMeans(INT_means_real)
      INT_se_real <- matrix(unlist(INT_se_real), ncol = 3, byrow = TRUE)
      INT_se_real <- colMeans(INT_se_real)
      All_means_real <- matrix(unlist(All_means_real), ncol = 15, byrow = TRUE)
      All_means_real <- colMeans(All_means_real)
      All_se_real <- matrix(unlist(All_se_real), ncol = 15, byrow = TRUE)
      All_se_real <- colMeans(All_se_real)
      ISI_means_pred <- matrix(unlist(ISI_means_pred), ncol = 5, byrow = TRUE)
      ISI_means_pred <- colMeans(ISI_means_pred)
      ISI_se_pred <- matrix(unlist(ISI_se_pred), ncol = 5, byrow = TRUE)
      ISI_se_pred <- colMeans(ISI_se_pred)
      INT_means_pred <- matrix(unlist(INT_means_pred), ncol = 3, byrow = TRUE)
      INT_means_pred <- colMeans(INT_means_pred)
      INT_se_pred <- matrix(unlist(INT_se_pred), ncol = 3, byrow = TRUE)
      INT_se_pred <- colMeans(INT_se_pred)
      All_means_pred <- matrix(unlist(All_means_pred), ncol = 15, byrow = TRUE)
      All_means_pred <- colMeans(All_means_pred)
      All_se_pred <- matrix(unlist(All_se_pred), ncol = 15, byrow = TRUE)
...
ggsave(
  ...
  plot = Final_plot_sub,
  device = "png",
  path = "~/Dropbox/ampOddClick/r-filesFran/Figures/Model_fit/EEG/",
  scale = 2,
  width = 15,
  height = 5,
  units = "cm",
