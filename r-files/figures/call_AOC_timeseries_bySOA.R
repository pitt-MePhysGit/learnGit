call_AOCtimeseries_bySOA <- function(what='mua',who='all', dpthStr='MUA',ganrm=T,dxdn=NA, ...){
  ## ============================================ by SOA
  ...
  if(!is.na(dxdn))
    tmpSub <- loadAllSessions( useInd, extn='mean_ISIbinOct',who='all',what=what,xnd=dxdn, ynd=NA, ganrm=ganrm)
  
  for (sx in SOAndx){
    tmp <- loadAllSessions( useInd, extn='mean_ISIbinOct',who='all',what=what,xnd=sx, ynd=NA, ganrm=ganrm)
    if(!is.na(dxdn))
      tmp <- tmp - tmpSub
  ...
  print('loaded sessions sucessfully...')
  ...
  fileName <- paste('AOC_timeseries_bySOA_',what,'_',who,'_',dpthStr,'.eps',sep='') 
  if (!is.na(dxdn))
    fileName <- paste('AOC_timeseries_bySOA_Delta_',dxdn,'_',what,'_',who,'_',dpthStr,'.eps',sep='') 
    
  eps.file(fileName, pdf=T, s=2, ratio=3/2)
  disc <- figure_AOC_timeseries_bySOA(tone, whoStr='lfp', ...)
  dev.off()
