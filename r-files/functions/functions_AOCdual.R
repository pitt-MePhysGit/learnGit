loadAOC <- function( dfline, chStr='frontoCentral',showCh=1,show=F,exp=NA, from=NA, to=NA, what='llp2', suaMethod='nev',
  ...
  if(!is.na(dfline)){
    exp    <- dfline$session
    exp2   <- exp
    if (identical( substr(what,1,3), 'ICA'))
      exp2    <- dfline$exp
  ...
  if (!is.na(chStr) ){# get lfps only if chStr is specified
    ...
    # check if loading ICA data:
    if (identical( substr(what,1,3), 'ICA')){
      load( paste(rda(), exp2, '/', exp2,'_',what,'.rda',sep=''),verbose=T )
      tone$weights <- ic$weights
      tone$A       <- ic$A
      tone$K       <- ic$K
      tone$W       <- ic$W
      tone$srt     <- ic$srt
    ...
    if (loadCC){ # new naming convention introduced for set2R and cross-correlation. Ultimately transition to this convention
      ...
      fileType <- '.mat'
      readRDA <- file.exists(paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[1],'.rda',sep=''))
      if (readRDA)
        fileType <- '.rda'
      
      print(paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[1],fileType,sep=''))
      if (identical(fileType,'.mat'))
        tmpLFP   <- t( readMat(paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[1],fileType,sep=''))[[1]] )
      
      if (identical(fileType,'.rda')){
        load( paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[1],fileType,sep=''),verbose=T )
        tmpLFP <- icdata
        rm(icdata)
      }
    }
    ...
    if(length(chStr)>1){
      for (chnd in 2:length(chStr)){
        if (!loadCC){
          ...
          print(                            paste(rda(), exp2,'_', exp2,'_',chStr[chnd],chextn,'.mat',sep=''))
          tone$lfp[,,chnd]   <- t( readMat( paste(rda(), exp2,'/', exp2,'_',chStr[chnd],chextn,'.mat',sep=''))[[1]] )
        }
        ...
        if (loadCC){
          ...
          print(                             paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[chnd],fileType,sep=''))
          if (identical(fileType,'.mat'))
            tone$lfp[,,chnd]   <- t( readMat(paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[chnd],fileType,sep=''))[[1]] )
          
          if (identical(fileType,'.rda')){
            load( paste(rda(), exp2, '/', exp2,'_',what,'_',chStr[chnd],fileType,sep=''),verbose=T )
            tone$lfp[,,chnd] <- icdata
            rm(icdata)
          }
        }
      }
    ...
    if (loadCC){
      ...
      print(paste(rda(), exp, '/', exp,'_',what,'_taxis',fileType,sep=''))
      if (identical(fileType,'.mat'))
        tone$taxis   <- t( readMat(paste(rda(), exp, '/', exp,'_',what,'_taxis',fileType,sep=''))[[1]] )
      
      if (identical(fileType,'.rda')){
        load( paste(rda(), exp, '/', exp,'_',what,'_taxis',fileType,sep=''),verbose=T )
        tone$taxis <- taxis
      }
    }
  ...
  if (readSUA){
    ...
    #eventFile     <-  paste( trda(), dfline$session, '/events.txt',sep='');
    eventFile     <-  paste( '/Volumes/Drobo5D3/EEG/EEGLab/ampOddClick/', dfline$session, '/events.txt',sep='');
