...
dataSet <- 'g'
chStr='frontoCentral'  
tdf <- df[which(df$dataSet==dataSet&df$animal==animal),]

tone        <- loadAmpOddClickAll( tdf , what=what, chStr=chStr,show=T)
...
eps.file( paste('EEG_SOAandAmp_',animal,'_',dataSet,'_',chStr,'_',what, '.eps',sep=''),pdf=T,s=2,ratio=3/2 )
...
for(n in 1:(Nbin-1)){
  ...
  disc   <- mean.plot( tone$lfp[valInd,,], tone$taxis, add=ifelse(n==1,F,T), sd=TRUE, traces=F, 
                       posFUN=na.mean, widthFUN=na.se, range=NULL,lwd=lwd, ylim=ylim,
                       xlim=xlim,col=col[n])
}
abline(h=0)
dev.off()
...
eps.file( paste('EEG_SOAandAmp2_',animal,'_',dataSet,'_',chStr,'_',what, '.eps',sep=''),pdf=T,s=2,ratio=3/2 )
Nbin <- length(unique(tone$xpp$ampInd))
col <- bluered.colors(Nbin)
for(n in 1:(Nbin)){
  ...
  disc   <- mean.plot( tone$lfp[valInd,,], tone$taxis, add=ifelse(n==1,F,T), sd=TRUE, traces=F, 
                       posFUN=na.mean, widthFUN=na.se, range=NULL, col=col[n],lwd=lwd, ylim=ylim,
                       xlim=xlim)
}
abline(h=0)
...
if (1==0){
  ...
  mn     <- tapply(tone$xpp$n1.r, list(tone$xpp$dB,tone$xpp$isiOct), na.mean )
  mnse  <- tapply(tone$xpp$n1.r, list(tone$xpp$dB,tone$xpp$isiOct), na.se )
  dbax   <- tapply(tone$xpp$dB, list(tone$xpp$dB), mean )
  isiax  <- tapply(tone$xpp$ISI, tone$xpp$isiOct, mean)
  
  
  N <- table(tone$xpp$isiOct)
  vi <- which(N>300)
  ...
  errorplot(log2(isiax[vi]),mn[1,vi], ey=mnse[1,vi], col='black', ylim=c(-60,10),lwd=2,pch=20)
  for (ax in 1:length(dbax))
    errorplot(log2(isiax[vi]),mn[ax,vi], ey=mnse[ax,vi], col=col[ax], add=T,lwd=2,pch=20)
  
  col <- soa.colors(6)
  errorplot(dbax,mn[,vi[1]], ey=mnse[,vi[1]], col='black', ylim=c(-60,10),lwd=2,pch=20)
  for (sx in 1:length(vi))
    errorplot(dbax,mn[,vi[sx]], ey=mnse[,vi[sx]], col=col[sx], add=T,lwd=2,pch=20)
  dev.off()
  ...
  vi <- which(N>100)
  mn     <- tapply(tone$xpp$n1.r, list(tone$xpp$dB,tone$xpp$isiOctFine), na.mean )
  mnse  <- tapply(tone$xpp$n1.r, list(tone$xpp$dB,tone$xpp$isiOctFine), na.se )
  dbax   <- tapply(tone$xpp$dB, list(tone$xpp$dB), mean )
  ...
  getNonSteady<-function(tn,lmbd=.3,U=.7,Off=0){
    subs <- which(tn$xpp$trialType==1 & tn$xpp$toneType==1)
    isi <- tn$xpp$ISI[subs]
    ...
    return(list(amp=N1amp,scl=N1scl)) 
  }
  ...
  subs <- which(tone$xpp$trialType==1 & tone$xpp$toneType==1)
  lm0 <- lm(tst1 ~ tone$xpp$dB[subs]+I(log(tone$xpp$ISI[subs])/log(2)) )
  image( tapply(lm0$resid,list(tone$xpp$dB[subs],tone$xpp$deltaAmpInd[subs]),mean), zlim=c(-.1,.1),col=heat.colors(100))
  lm0 <- lm(tst2 ~ tone$xpp$dB[subs]+I(log(tone$xpp$ISI[subs])/log(2)))
  image( tapply(lm0$resid,list(tone$xpp$dB[subs],tone$xpp$deltaAmpInd[subs]),mean ), zlim=c(-.1,.1),col=heat.colors(100))
  ...
  cumFit <- callSTPSP(tone,                 DVstr='p1b.r', gd=T, show=TRUE, showCh=1, who='random', BYstr='exp',
                      FUN=cumsum1, minISI=.190, maxISI=12.8, startFromScratch=T)
  cumFit$modelFit$par
  
  
  ## scale R/EEGamp with tone amplitude
  sclFit <- callSTPSP(tone,                 DVstr='n1', gd=T, show=TRUE, showCh=1, who='random', BYstr='exp', 
                      FUN=cumsumscl, minISI=.190, maxISI=12.8, startFromScratch=T)
  sclFit$modelFit$par
  
  
  ## scale expended Fraction with tone amplitude
  ampFit <- callSTPSP(tone,                 DVstr='n1', gd=T, show=TRUE, showCh=1, who='random', BYstr='exp', 
